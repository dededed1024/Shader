...
- **발신자 토글 (나 / 상대)** 로 상대방이 보낸 답장처럼 왼쪽 회색 말풍선도 만들 수 있습니다.

## 발신자 전환 (답장 흉내내기)

입력창 왼쪽의 **`나`** 버튼을 누르면 **`상대`** 로 바뀝니다. 그 상태에서 메시지를 보내면 왼쪽 회색 말풍선(iMessage 받은 메시지 스타일)으로 표시돼요.

내부적으로는 메시지 파일 앞에 다음 frontmatter 가 자동으로 붙습니다:
...
따라서 이미 만들어진 메시지를 열어서 위 frontmatter 를 추가/제거하는 것만으로도 발신자를 바꿀 수 있습니다.
